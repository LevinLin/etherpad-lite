...
describe('importexport.js', function () {
  ...
  // A DataTransfer object is used to create a mutable FileList, which as of 2021-02-22 cannot be
  // directly created. For details, see:
  ...
  const newDataTransfer = () => {
    try {
      ...
      try {
        // Workaround for Firefox < 62.
        return new ClipboardEvent('').clipboardData;
      } catch (err) {
        // Chrome < 60 doesn't support either approach.
        return null;
      }
    }
  };
  ...
  before(async function () {
    ...
    if (newDataTransfer() == null) return this.skip();
  });
